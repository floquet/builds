...
# source         /repos/github/builds/SpWx/scripts-shell/SpWx-copy-build.sh 2>&1 | tee -a ${scratch}/copy-spwx.txt
# source /Volumes/repos/github/builds/SpWx/scripts-shell/SpWx-copy-build.sh 2>&1 | tee -a ${scratch}/copy-spwx.txt
source ${repo_build}/scripts-spack/shared/common-header.sh

export buildSeconds=${SECONDS}
export dirCopyFrom="/atacama/SpWx/source/"
export dirCopyTo="${scratch}/SpWx"

new_step "Set up source and sink repos"
sub_step_counter=0

    sub_step "\${dirCopyTo}=${dirCopyTo}"

    sub_step "\${dirCopyFrom}=${dirCopyFrom}"
...
sub_step_counter=0
    ...
    sub_step 'rm -rf ${dirCopyTo}'
              rm -rf ${dirCopyTo}

    sub_step "mkdir -p ${dirCopyTo}"
              mkdir -p ${dirCopyTo}

    sub_step "cd ${dirCopyTo}"
              cd ${dirCopyTo}
    ...
    sub_step "cp -a ${dirCopyFrom} ."
              cp -a ${dirCopyFrom} .

    sub_step 'export localSpWx="${dirCopyTo}"'
              export localSpWx="${dirCopyTo}"
